...
function getAwsClient(env: Bindings) {
...
// Auth Helper
async function checkAuth(c: any, workspace: any): Promise<boolean> {
...
app.get('/api/workspaces/:id/exists', async (c) => {
  ...
  const workspace = await c.env.DB.prepare(
    'SELECT expires_at, password_hash FROM workspaces WHERE id = ?'
  ).bind(id).first()
    ...
    if (existing) {
      ...
      if (existingExpiry > now) {
        ...
        await c.env.DB.prepare(
          'UPDATE workspaces SET created_at = ?, expires_at = ?, password_hash = ?, salt = ? WHERE id = ?'
        )
      ...
      await c.env.DB.prepare(
        'INSERT INTO workspaces (id, created_at, expires_at, password_hash, salt) VALUES (?, ?, ?, ?, ?)'
      )
...
app.post('/api/workspaces/:id/auth', async (c) => {
  ...
  const workspace = await c.env.DB.prepare(
    'SELECT password_hash, salt FROM workspaces WHERE id = ?'
  ).bind(id).first()
  ...
  const hash = await hashPassword(password, workspace.salt as string)
...
app.post('/api/uploads/presign', async (c) => {
  ...
  try {
    ...
    // We MUST use signQuery: true to put authentication in the URL for the browser
...
app.post('/api/uploads/multipart/initiate', async (c) => {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`R2 Initiate Error: ${await response.text()}`)
...
app.post('/api/uploads/multipart/complete', async (c) => {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`R2 Complete Error: ${await response.text()}`)
...
app.post('/api/uploads/multipart/abort', async (c) => {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`R2 Abort Error: ${await response.text()}`)
